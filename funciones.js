...
const creari = (usuario) => {
    ...
    if (!cur) {
        ...
        let est = listaUsuarios.find(buscar => buscar.id == usuario.id && buscar.nombre == usuario.nombre && buscar.tipo == "aspirante")
        if (!est) {
...
const actualizar = (id) => {
    ...
    if(!coordinador){
        console.log("no tienes permiso para modificar los cursos")
    }
    else{
        if(!encontrado){
...
const actualizardatos = (nm, id, email, tel) => {
    listaru();
    let actualizar = listaUsuarios.find(cambiar => cambiar.id == id);
    if(!actualizar){
        console.log("Este usuario no existe");
    }
    else{
        actualizar.nombre = nm;
        actualizar.correo = email;
